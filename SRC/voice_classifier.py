...
class VoiceClassifier:
    ...
    def conv_net_2d(self):
        ...
        input_layer = get_melspectrogram_layer(input_shape=input_shape, n_mels=128, pad_end=True, n_fft=512,
                                               win_length=400, hop_length=160, sample_rate=self.sr,
        ...
        x_layer = layers.Conv2D(16, kernel_size=(5, 5), activation='relu', padding='same', name='conv2d_relu_0')(
            x_layer)
        x_layer = layers.MaxPooling2D(pool_size=(2, 2), padding='same', name='max_pool_2d_1')(x_layer)

        # Was left out
        x_layer = layers.Conv2D(16, kernel_size=(3, 3), activation='relu', padding='same', name='conv2d_relu_1')(
            x_layer)
        x_layer = layers.MaxPooling2D(pool_size=(2, 2), padding='same', name='max_pool_2d_2')(x_layer)

        x_layer = layers.Conv2D(32, kernel_size=(3, 3), activation='relu', padding='same', name='conv2d_relu_2')(
            x_layer)
        x_layer = layers.MaxPooling2D(pool_size=(2, 2), padding='same', name='max_pool_2d_3')(x_layer)
        # Was left out end

        x_layer = layers.Conv2D(32, kernel_size=(3, 3), activation='relu', padding='same', name='conv2d_relu_3')(
            x_layer)
    ...
    def train(self):
        ...
        wav_train, wav_val, label_train, label_val = train_test_split(wav_train, label_train, test_size=0.1,
                                                                      random_state=0)
        ...
        cp = ModelCheckpoint('snapshots/{}.h5'.format(self.model_type), monitor='val_loss', save_best_only=True,
    ...
    def evaluate(self, voice_classifier_snapshot=None):
        ...
        model = load_model(voice_classifier_snapshot,
                           custom_objects={'STFT': STFT, 'Magnitude': Magnitude, 'ApplyFilterbank': ApplyFilterbank,
