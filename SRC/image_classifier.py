...
from tensorflow.keras.layers import Input, AveragePooling2D, Conv2D, Dropout, LeakyReLU, BatchNormalization, Dense, \
    Flatten
...
class ImageClassifier:
    ...
    def train(self, snapshot_path):
        ...
        modelcheckpoint_callback = ModelCheckpoint(snapshot_path, monitor="val_categorical_accuracy",
        ...
        # Data augmentation
        datagen = ImageDataGenerator(preprocessing_function=(lambda img: (img - 127.5) / 127.5), width_shift_range=0.05,
                                     height_shift_range=0.05, rotation_range=5, zoom_range=[0.95, 1.1],
                                     brightness_range=[0.85, 1.15])
        train_gen = datagen.flow(self.dataset.x_train, to_categorical(y_train), batch_size=self.batch_size)

        # Train model
        self.classifier.fit(train_gen, batch_size=self.batch_size, epochs=self.epochs,
                            callbacks=[earlystopping_callback, modelcheckpoint_callback],
                            validation_data=(self.dataset.x_val, to_categorical(y_val)))
